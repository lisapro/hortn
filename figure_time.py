...
def get_z(df_brom,param):
    return df_brom[param][util.start_time:util.stop_time,:,util.release_col].values #.T
...
def fig7(): #'Figure_Horten_Time_7col.png'
    ...
    #.timedelta(days=35+23, hours=21, minutes=43, seconds=11.946760192)  # adds up to 365 days   
    #print (df_brom.time.values[46380])
    ...
    [axis.set_ylabel('Distance,m') for axis in [ax00,ax01,ax02,ax03]]
    #ax04.set_xlabel('Day,time')
    ax04.set_xlabel('Month,Day')    
    
    plt.savefig('Figure_Horten_Time_7col.png')
    #plt.show()
