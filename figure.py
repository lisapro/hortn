...
def get_z(df_brom,param):
    return df_brom[param][util.dist_time,:,:].values.T

def get_levels_fig8(param):
    z  = get_z(df_brom,param)
    sed_levels = util.get_levels_1_arr(z[:,sed2-1:]) 
    levels = util.get_levels_1_arr(z[:,:sed2])
    return levels,sed_levels

#def plot_param_time('O2',ax00_1,ax00_1_cb,ax00_1_sed,ax00_1_sed_cb):
#    pass
...
def plot_param_dist(param,z,axis,axis_cb):
    ...
    cmap =  plt.get_cmap('cubehelix') #palettable.wesanderson.Zissou_5.mpl_colormap

    CS_1 = axis.contourf(X,Y, z[:,:].T, levels = levels,extend="both",cmap = cmap) 
...
def fig7(): #'Figure_horten.png'
    ...
    x = df_brom.i.values
    x = x - x[util.release_col] #- int(x[1]/2) # normalize dist by middle column and midlle of middle column 
    y = df_brom.z.values
    sed = 5 
    y_sed = ((y - y[sed2])*100)   
    ...
    [axis.set_ylabel('Depth,m') for axis in [ax00,ax01,ax02,ax03]]

    plt.savefig('Figure_horten_dist_depth.png')
    #plt.show()
